...
local function do_exit_afk(code)
    ...
    lua_thread.create(function()
        if CHAT_NOTIFY then
            sampAddChatMessage(string.format("[AntiAFK_Leave] Menunggu %d detik sebelum keluar dari AFK...", WAIT_DELAY / 1000), 0xFFA500)
        end
        wait(WAIT_DELAY)
        sampSendChat(cmd)
        if CHAT_NOTIFY then
            sampAddChatMessage(string.format("[AntiAFK_Leave] Mengirim: %s", cmd), 0x1CD031)
        end
    end)
end
