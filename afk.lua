...
local function do_exit_afk(code)
    ...
    lua_thread.create(function()
        if CHAT_NOTIFY then
            sampAddChatMessage(string.format("[AntiAFK_Leave] Menunggu %d detik...", WAIT_DELAY / 1000), 0xFFA500)
        end
        wait(WAIT_DELAY)
        if ENABLED then
            sampSendChat(cmd)
            sampAddChatMessage("[AntiAFK_Leave] Keluar dari AFK.", 0x1CD031)
        end
    end)
end
...
function stop()
    ENABLED = false
    sampAddChatMessage("[AntiAFK_Leave] Dinonaktifkan oleh Loader.", 0xFF3333)
end

function main()
    repeat wait(100) until isSampAvailable()
    sampAddChatMessage("[AntiAFK_Leave] Aktif otomatis.", 0x1CD031)
